<script lang="ts">
  ...
  import { currentSong, setColor, togglePlay } from '$lib/store/currentPlaying';
  ...
  let color = $state(null) as string | null;
  const pagination = data.pagination;
  let hasLiked = $state(data.hasliked);
  let tracks = $state(data.playlist.tracks.items.map((item) => item.track!));
