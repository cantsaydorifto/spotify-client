<script lang="ts">
  ...
  const userSavedTracks = data.userSavedTrackRes;
  const color = data.color.dominantColor;
  const pagination = data.pagination;
  let tracks = $state(data.userSavedTrackRes.items.map((item) => item.track!));
...
<div class="content">
  ...
  {#if pagination.hasMoreTracks}
    <div class="showMoreBtn">
      <button onclick={() => loadMoreTracks()}>Show More</button>
    </div>
  {/if}
</div>
