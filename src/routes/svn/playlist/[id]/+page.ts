...
export const load: PageLoad = async ({ fetch, params }) => {
  const res = await fetch('/api/saavn/playlists?id=' + params.id);
  if (!res.ok) throw error(res.status, 'Album not found');
  const resJson = (await res.json()) as SaavnApiPlaylistResponse;
  const playlist = resJson.data;
  let color: string | null = null;
  if (playlist.image.length > 0) {
    const colorRes = await fetch('/api/color?image=' + playlist.image[0].link);
    if (colorRes.ok) {
  ...
  return {
    playlist,
    color,
    tracks: playlist.songs.map((el) => convertSaavnSongToTrackObjectFull(el))
  };
};
